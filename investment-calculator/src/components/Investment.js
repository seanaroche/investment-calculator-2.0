...
export default function Investments() {
  ...
  const handleSubmit = (event) => {
    ...
    if (values.contribution.includes(",")) {
      annualContributionArray = values.contribution.split(",")
    }
    else {
    ...
    if (annualContributionArray.length !== 0) {
      for (let i = 1; i <= numOfYears; i++) {
        total += parseInt(annualContributionArray[i]);
        total += total * rate;
        data.push({
          year: i,
          contribution: parseInt(annualContributionArray[i]),
          balance: total,
        });
      }
    }
